      ...
      <section class="auth">
        ...
        <form class="auth-card glass" id="login-form" autocomplete="on" novalidate>
          ...
          ${UI.field('Senha', `
            ...
            <button type="button" class="eye" data-eye aria-label="Mostrar senha">${showPass ? '🙈' : '👁️'}</button>`)}

          ${error ? `<div class="err">${UI.esc(error)}</div>` : ''}

          <button type="submit" class="btn btn-primary btn-block">
            Entrar ${isChild ? 'como filho(a)' : 'como responsável'} <span>→</span>
          </button>
        ...
        <div class="row" style="justify-content:center;gap:10px">
          <button class="btn btn-ghost btn-sm" data-theme-toggle>
            ${Store.theme() === 'dark' ? '☀️ Tema claro' : '🌙 Tema escuro'}
          </button>
        </div>
      </section>`;
